...
mail.sendMailByAPI = (emailSubject, emailBody, officeId, referId) => {
    log.info("mail.js, Handlers: sendMailByAPI")
    ...
    const postData = {
        ...
        "instant": "Y",
        "notificationId": mailConfig.toMailId,
    ...
    log.info("mail.js, Handlers: sendMailByAPI, calling API: " + mailConfig.url)
    request.post(mailConfig.url, postData,(error, res, body) => {
        if (error) {
          log.error("mail.js, Handlers: sendMailByAPI " + error)
          return
        }
        log.info("mail.js, Handlers: sendMailByAPI, statusCode:" + res.statusCode + JSON.stringify(body))
      }) 
}
