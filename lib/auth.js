...
auth.loginV2Handler = function(req,res,next){
    ...
    //check if username password present
    if(typeof(login_name) != "string" || typeof(login_password) != "string"){
        res.send(400, {error: "No Data found"});
        return next();
    }
    const loginQuery = require('./sql').login.v2.query;
    commonHandler(req,res,next,loginQuery,[login_name,login_password]);
}
