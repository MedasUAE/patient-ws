...
sql.appointment = {};
sql.appointment.v1 = {};
sql.appointment.v1.columns = ["DATE_FORMAT(APT.appoint_date,'%Y-%m-%d') appoint_date", 'APT.appoint_hr', 'APT.op_number', 'APT.doctors_id', 
                            'DS.doctors_name', 'OD.office_Name','DPS.department_name','APT.department_id', 
                            'APT.office_id', 'APT.id', 'APT.appoint_type', 'APT.consult_id'];
sql.appointment.v1.query = "SELECT " + sql.appointment.v1.columns.join(',') + 
                        " FROM appointments APT " +
                        "LEFT JOIN doctors_setup DS ON APT.doctors_id = DS.doctors_id " +
                        "LEFT JOIN department_setup DPS ON APT.department_id = DPS.department_id " +
                        "LEFT JOIN office_details OD ON APT.office_id = OD.office_Id " +
                        "WHERE APT.op_number = ? AND APT.appoint_date >= ? AND APT.cancel_status = 'N'";

// patient consult v1 query by op_number
sql.consult = {};
sql.consult.v1 = {};
sql.consult.v1.columns = ['DC.consult_id', 'DC.consult_date', 'DS.doctors_name', 'OD.office_Name', 'DC.office_id', 'DC.encounter_type', 'DPS.department_name'];
sql.consult.v1.query =  "SELECT "+ sql.consult.v1.columns.join(',') +
                        ...
                        " LEFT JOIN department_setup DPS ON DC.department_id = DPS.department_id" +
                        " WHERE DC.op_number = ?" +
                        " ORDER BY DC.consult_date DESC";

sql.consult.v2 = {}
sql.consult.v2.columns = ['max(consult_id) as consult_id']
sql.consult.v2.query =  "SELECT " + sql.consult.v2.columns.join(',') +
                        " FROM doctor_consult " +
                        " WHERE op_number = ?";
                        
//tele consult URL by op_number
sql.tele = {};
sql.tele.v1 = {};
sql.tele.v1.columns = ['patient_url'];
sql.tele.v1.query =  "SELECT "+ sql.tele.v1.columns.join(',') +
...
// patient laborders v1 query by op_number
sql.laborder = {};
sql.laborder.v1 = {};
sql.laborder.v1.columns = [
                            'test_setup.test_Name', 
                            'consult_labtest.labtest_id', 
                            'consult_labtest.entered_date', 
                            'test_setup.profile_type', 
                            'doctor_consult.office_id', 
                            'consult_labtest.consult_id',
                            'consult_labtest.samp_collect',
                            'test_details.dispatch_status',
                        ];
sql.laborder.v1.query = "SELECT "+ sql.laborder.v1.columns.join(',') +
                        " FROM consult_labtest" +
                        " JOIN test_setup ON test_setup.test_Id = consult_labtest.labtest_id" +
                        " JOIN doctor_consult ON doctor_consult.consult_id = consult_labtest.consult_id" +
                        " INNER JOIN test_details ON test_details.consult_id = consult_labtest.consult_id" +
                        " WHERE doctor_consult.op_number = ? AND doctor_consult.refered_consult_id IS NULL" + 
                        " GROUP BY test_setup.test_Name;";
...
// patient labtests v2 query by consult_id
sql.labresult.v2 = {};
sql.labresult.v2.columns = ['a.test_id', 'a.test_name', 'c.parameter_id', 'c.parameter_name', 'd.id mappingid', 
                            'd.param_mapping_name' , 'test_result', 'fnormal_Value', 'f.measure']
sql.labresult.v2.query =     "SELECT "+ sql.labresult.v1.columns.join(',') +
                            " FROM test_setup a" + 
                            " JOIN test_parameter b ON a.test_id = b.test_id" +
                            " JOIN test_parameter_setup c ON b.parameter_id = c.parameter_id" +
                            " JOIN test_parameter_mapping d ON d.parameter_id = b.parameter_id" +
                            " JOIN test_Details e ON e.test_id = a.test_id" +
                            " JOIN test_results f ON f.test_detailsid = e.test_detailsid AND e.test_id= f.test_id AND f.parameter_id = c.parameter_id AND f.param_mapping_id = d.id" +
                            " WHERE e.consult_id = ? AND e.test_id = ? AND b.office_id = ? AND dispatch_status='Y'"

sql.labresult.v2.queryForProfile =  "SELECT "+ sql.labresult.v1.columns.join(',') +
                                    " FROM test_setup a" + 
                                    " JOIN test_parameter b ON a.test_id = b.test_id" +
                                    " JOIN test_parameter_setup c ON b.parameter_id = c.parameter_id" +
                                    " JOIN test_parameter_mapping d ON d.parameter_id = b.parameter_id" +
                                    " JOIN test_Details e ON e.test_id = a.test_id" +
                                    " JOIN test_results f ON f.test_detailsid = e.test_detailsid AND e.test_id= f.test_id AND f.parameter_id = c.parameter_id AND f.param_mapping_id = d.id" +
                                    " WHERE e.consult_id = ? AND e.profile_id = ? AND b.office_id = ? AND dispatch_status='Y'"
...
sql.doctors = {};
sql.doctors.v1 = {};
sql.doctors.v1.columns = ['ds.doctors_id', 'doctors_name', 'ds.doctor_type', 'department_name', 'od.office_Name', 'deps.department_id']
...
sql.doctors.v2 = {};
sql.doctors.v2.columns = ['ds.doctors_id', 'doctors_name', 'ds.doctor_type', 'department_name', 'od.office_Name', 'od.office_Id', 'deps.department_id']
...
sql.offices.v1.query =  "SELECT " + sql.offices.v1.columns.join(',') +
                        " FROM office_details" +
                        " WHERE active_Status = 'Y' AND show_in_mobile = 'Y';"
