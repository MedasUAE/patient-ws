...
/* get service version 1 list by office */
...
/* get service version 2 list by office */
master.servicesV2Handler = function(req,res,next){
    log.debug("master.js, Handlers: servicesV2Handler");
    console.log(req.query);
    let type  = req.query.type; //type = service || profile
    ...
    try {
        let result = []
        if(type == 'service')
            result = JSON.parse(fs.readFileSync('./data/service.v2.json', 'utf8'));
        else if(type == 'profile')
            result = JSON.parse(fs.readFileSync('./data/profile.v2.json', 'utf8'));
