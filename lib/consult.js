...
consult.labResultV2Handler = async function(req,res,next) {
    ...
    const radiologyQuery = require('./sql').radiology.v1.query;
    if(!req.params.consult_id && !req.params.test_id) res.send(400, {error: "consult_id/test_id missing"});
    ...
    */
    ...
    try {
        //get result from db
        let labResult = await execParamQuery(labQuery, [req.params.consult_id, req.params.test_id]);  
        let radiologyResult = await execParamQuery(radiologyQuery, [req.params.consult_id]);  
