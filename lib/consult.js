...
consult.bookAppointmentV2Handler = async function(req,res,next) {
    ...
    const { patient_name, doctors_id, mobile } = req.query;
    const execParamQuery = require('./mysql').execParamQuery;
    ...
    try {
        ...
        const doctorsQuery = require('./sql').doctors.v3.query;
        const doctorProfile = await execParamQuery(doctorsQuery, [doctors_id]);
        if(!doctorsProfile) {res.send(400, {error: "mobile missing"});return next();}
