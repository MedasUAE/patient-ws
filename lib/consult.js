...
consult.labOrderV1Handler = async function(req,res,next) {
    ...
    /* commonHandler to get insurar from db 
        @params:    query: labOrder.v1.query, 
                    params: [req.params.op_number]
    */
    return commonHandler(req,res,next,labOrderQuery.v1.query, [req.params.op_number]);
}
...
consult.labResultV2Handler = async function(req,res,next) {
    ...
    let labQuery = require('./sql').labresult.v2.query;
    const radiologyQuery = require('./sql').radiology.v1.query;
    if(!consult_id || !test_id || !office_id || !profile_type) res.send(400, {error: "consult_id/test_id/office_id/profile_type missing"});

    // different query to get result in case profile
    (profile_type == 'Y') ? labQuery = require('./sql').labresult.v2.queryForProfile : labQuery;
    ...
    */
    ...
    try {
        ...
        let radiologyResult = await execParamQuery(radiologyQuery, [consult_id]);  
...
/*
    handler for getLatestConsultID
    @params: op_number
    version: v1
*/
consult.getLatestCosultID = async function(req, res, next) {
    log.debug("consult.js, Handlers: getLatestCosultID");
    const consultQuery = require('./sql').consult;
    if(!req.params.op_number) res.send(400, {error: "op_number missing"});
    /* commonHandler to get insurar from db 
        @params:    query: labOrder.v1.query, 
                    params: [req.params.op_number]
    */
    return commonHandler(req,res,next,consultQuery.v2.query, [req.params.op_number]);
}
